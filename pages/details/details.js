...
Page({
  ...
  onShow() {
    ...
    for (let i in objLists) {
      if (objLists[i].num == obj.num) {
        this.setData({
          isTop: objLists[i].top
        })
      }
    }
    this.setData({
      expressNumber: obj.num
    })
    if (obj.key) {
      this.setData({
        expressType: obj.name
      })
  ...
  getMessageMaiDao() {
    const params = {
      data: {
        logisticsNo: this.data.expressNumber
      },
      method: 'GET'
    }
    req.getMDMessage(params, data => {
      if (data.status != 1 || data.data === null || data.data.length || data.data.logisticsNo === null) {
        ...
        const imageUrl = data.data.imageUrl || '../../image/holder.png';
        this.setData({
          imageUrl,
          title: data.data.productTitle,
          amount: data.data.productNumber
        })
      }
    }, data => {}, data => {
      util.hideToast()
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (expressChangeList.length === 0) {
        expressChangeList[0] = {
          ...
          list: [{
            "time": time,
            "acceptTime": data[i].acceptTime,
            "acceptStation": data[i].acceptStation
          }]
        }
      } else if (expressChangeList.length > 0) {
        if (expressChangeList[length - 1].date === date) {
          expressChangeList[length - 1].list.push({
            "time": time,
            "acceptTime": data[i].acceptTime,
            "acceptStation": data[i].acceptStation
          })
        } else {
          expressChangeList[length] = {
            "date": date,
            "week": week,
            list: [{
              "time": time,
